...
// hack to test RUN command and refreshing screen afterwards
void cmd_run(editor* ed) {
    printf("RUN");

    //int *l2 = ed->buf_.lb_.curr_;
    //int l3 = *l2;

    char* fname = ed->buf_.fname_;      // this is name of current file
   ...
   // UI(ed);
    ...
    //trying to work out cursor line

    //printf("GOT LINE: %d",l3);
    ...
    // return from runtime engine here
    // SCR(ed);
    // UI(ed);
    // TB(ed);
